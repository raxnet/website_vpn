...
ul {
    ...
    margin: 40px auto;
    max-width: 1200px;
    margin-top: 50px; /* Menambahkan jarak atas untuk memberikan ruang */
}
        .server-card {
            background-color: #fff;
            padding: 15px;
        ...
        if ($result->num_rows > 0) {
            while ($server = $result->fetch_assoc()) {
                ...
                if($result_akun) {
                    ...
                    echo "<a href='../user/proses_vmess?server_id=" . $server['id'] . "'><button class='create-account-btn'>Buat Akun</button></a>";
                    echo "</li>";
                }
            }
        } else {
